...
if ( ! defined( 'TRX_UTILS_VERSION' ) ) {
	define( 'TRX_UTILS_VERSION', '3.2.2' );
}
...
if (!function_exists('trx_utils_sc_init')) {
    ...
    function trx_utils_sc_init() {
        ...
        // Include widgets
        require_once 'shortcodes/core.shortcodes.php';
    }
}
...
if ( !function_exists( 'trx_utils_load_scripts_front' ) ) {
	...
	function trx_utils_load_scripts_front() {
		...
		wp_enqueue_script( 'trx_utils', trx_utils_get_file_url('js/core.googlemap.js'), array('jquery'), null, true );
...
if (!function_exists('trx_utils_get_privacy_text')) {
    function trx_utils_get_privacy_text() {
        ...
        if(function_exists('diveit_get_theme_option'))
            $privacy_text = diveit_get_theme_option( 'privacy_text' );
...
if (!function_exists('diveit_add_fields_in_user_profile')) {
    function diveit_add_fields_in_user_profile( $user ) {
        ?>
            ...
            if (is_array($social_list) && count($social_list) > 0) {
                foreach ($social_list as $soc) {
                    ...
                    if (!empty($sn)) {
                        ?>
                        <tr>
                            <th><label for="user_<?php echo esc_attr($sn); ?>"><?php diveit_show_layout(diveit_strtoproper($sn)); ?>:</label></th>
                            <td><input type="text" name="user_<?php echo esc_attr($sn); ?>" id="user_<?php echo esc_attr($sn); ?>" size="55" value="<?php echo esc_attr(get_the_author_meta('user_'.($sn), $user->ID)); ?>" />
                                <span class="description"><?php echo sprintf(esc_html__('Please, enter your %s link', 'trx_utils'), diveit_strtoproper($sn)); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
...
if (!function_exists('diveit_footer_add_scroll_to_top')) {
    add_action('wp_footer',								'diveit_footer_add_scroll_to_top', 1);
    function diveit_footer_add_scroll_to_top() {
...
if (!function_exists('trx_utils_set_post_likes')) {
    ...
    function trx_utils_set_post_likes($id=0, $count=-1) {
        global $wp_query;
        if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
        $count_key = trx_utils_storage_get('options_prefix').'_post_likes_count';
        update_post_meta($id, $count_key, $count);
...
if(!function_exists('trx_utils_filter_options')){
    add_filter('diveit_filter_options', 'trx_utils_filter_options');
...
if (!function_exists('trx_utils_widgets_disable_block_editor')) {
    ...
    function trx_utils_widgets_disable_block_editor() {
        if ( function_exists('diveit_get_theme_option') && diveit_get_theme_option( 'disable_widgets_block_editor' ) == 'yes' ) {
