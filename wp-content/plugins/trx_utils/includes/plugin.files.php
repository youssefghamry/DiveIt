...
if (!function_exists('trx_utils_get_theme_option')) {
    function trx_utils_get_theme_option($prm) {
        if (function_exists('diveit_get_theme_option')) {
            return diveit_get_theme_option($prm);
        }
...
// Set theme variable
if (!function_exists('trx_utils_storage_set')) {
    function trx_utils_storage_set($var_name, $value) {
        global $TRX_UTILS_STORAGE;
        $TRX_UTILS_STORAGE[$var_name] = $value;
    }
}
...
// or at the end if the needle is not found or not supplied.
// Modifies $haystack in place.
if ( ! function_exists( 'trx_utils_array_before' ) ) {
    function trx_utils_array_insert_before( &$haystack, $needle, $stuff ) {
        if ( ! is_array( $haystack ) ) {
            return -1;
        }
