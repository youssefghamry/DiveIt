...
// Theme init
if (!function_exists('diveit_widget_calendar_theme_setup')) {
	add_action( 'diveit_action_before_init_theme', 'diveit_widget_calendar_theme_setup', 1 );
	function diveit_widget_calendar_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('diveit_exists_visual_composer') && diveit_exists_visual_composer())
			add_action('diveit_action_shortcodes_list_vc','diveit_widget_calendar_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('diveit_widget_calendar_load')) {
	add_action( 'widgets_init', 'diveit_widget_calendar_load' );
	function diveit_widget_calendar_load() {
		register_widget('diveit_widget_calendar');
	}
}

// Widget Class
class diveit_widget_calendar extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_calendar', 'description' => esc_html__('Calendar for posts and/or Events', 'trx_utils'));
		parent::__construct( 'diveit_widget_calendar', esc_html__('DiveIt - Advanced Calendar', 'trx_utils'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = diveit_get_calendar(false, 0, 0, array('post_type'=>$post_type));

		if (!empty($output)) {
	
			// Before widget (defined by themes)
			diveit_show_layout($before_widget);
			
			// Display the widget title if one was input (before and after defined by themes)
			if ($title) diveit_show_layout($before_title . $title . $after_title);
	
			diveit_show_layout($output);
			
			// After widget (defined by themes)
			diveit_show_layout($after_widget);
		}
	}
	...
	function form($instance) {
		...
		$posts_types = diveit_get_list_posts_types(false);
		?>
...
*/
if ( !function_exists( 'diveit_sc_widget_calendar' ) ) {
	function diveit_sc_widget_calendar($atts, $content=null){	
		$atts = diveit_html_decode(shortcode_atts(array(
			// Individual params
			"title" => "",
			"weekdays" => "short",
			// Common params
		...
		extract($atts);
		$type = 'diveit_widget_calendar';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_calendar' 
								. (diveit_exists_visual_composer() ? ' vc_widget_calendar wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, diveit_prepare_widgets_args(diveit_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_calendar', 'widget_calendar') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('diveit_shortcode_output', $output, 'trx_widget_calendar', $atts, $content);
	}
	add_shortcode("trx_widget_calendar", "diveit_sc_widget_calendar");
}


// Add [trx_widget_calendar] in the VC shortcodes list
if (!function_exists('diveit_widget_calendar_reg_shortcodes_vc')) {
	//add_action('diveit_action_shortcodes_list_vc','diveit_widget_calendar_reg_shortcodes_vc');
	function diveit_widget_calendar_reg_shortcodes_vc() {
