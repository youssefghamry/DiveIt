	...
	if (typeof(vc) != 'undefined' && typeof(vc.shortcode_view) != 'undefined') {
		...
		window.VcTrxColumnsView = vc.shortcode_view.extend({
			changeShortcodeParams:function (model) {
				...
				if (_.isObject(params) && _.isString(params.count)) {
					diveit_vc_columns_width(this.$el, params.count);
				}
			},
			changedContent: function(view) {
		...
		window.VcTrxColumnItemView = vc.shortcode_view.extend({
			changeShortcodeParams:function (model) {
				...
				if (_.isObject(params) && _.isString(params.span)) {
					...
					diveit_vc_columns_width(this.$el.parents('.trx_sc_columns'));
				}
			},
			changedContent: function(view) {
				window.VcTrxColumnsView.__super__.changedContent.call(view);
		...
		jQuery('body').on('change', 'select.post_type', function () {
			...
			diveit_admin_fill_categories(fld, cat_fld, cat_lbl);
		});
	}
});
...
function diveit_vc_columns_width(columns) {
	var count = arguments[1] ? arguments[1] : 0;
	var c = '';
	if (count > 0) 
		columns.removeClass('trx_columns_count_1 trx_columns_count_2 trx_columns_count_3 trx_columns_count_4 trx_columns_count_5 trx_columns_count_6 trx_columns_count_7 trx_columns_count_8 trx_columns_count_9 trx_columns_count_10 trx_columns_count_11 trx_columns_count_12').addClass('trx_columns_count_'+count);
	else {
		c = columns.attr('class');
		if ((pos = c.indexOf('trx_columns_count_')) >= 0)
			count = Math.max(1, c.substr(pos+18, 2));
	}
	columns.find('> .wpb_element_wrapper > .vc_container_for_children > .trx_sc_column_item').each(function(idx) {
		...
		if ((pos = c.indexOf('trx_columns_span_')) >= 0) {
			var span = Math.max(1, c.substr(pos+17, 2));
			w = w*span + m*(span-1);
		}
