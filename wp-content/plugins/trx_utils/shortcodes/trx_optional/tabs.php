...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('diveit_sc_tabs_theme_setup')) {
	add_action( 'diveit_action_before_init_theme', 'diveit_sc_tabs_theme_setup' );
	function diveit_sc_tabs_theme_setup() {
		add_action('diveit_action_shortcodes_list', 		'diveit_sc_tabs_reg_shortcodes');
		if (function_exists('diveit_exists_visual_composer') && diveit_exists_visual_composer())
			add_action('diveit_action_shortcodes_list_vc','diveit_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('diveit_sc_tabs')) {	
	function diveit_sc_tabs($atts, $content = null) {
		...
		diveit_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = diveit_storage_get_array('sc_tab_data', 'titles');
		...
		$tabs_output .= $titles_output
			...
			. ($content)
			.'</div>';
		return apply_filters('diveit_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	add_shortcode("trx_tabs", "diveit_sc_tabs");
}


if (!function_exists('diveit_sc_tab')) {	
	function diveit_sc_tab($atts, $content = null) {
		if (diveit_in_shortcode_blogger()) return '';
		extract(diveit_html_decode(shortcode_atts(array(
			// Individual params
		...
		diveit_storage_inc_array('sc_tab_data', 'counter');
		$counter = diveit_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : diveit_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = diveit_storage_get_array('sc_tab_data', 'titles');
		...
		diveit_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (diveit_param_is_on(diveit_storage_get_array('sc_tab_data', 'scroll')) 
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(diveit_storage_get_array('sc_tab_data', 'height') != '' ? diveit_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' 
					: '')
				...
				. (diveit_param_is_on(diveit_storage_get_array('sc_tab_data', 'scroll')) 
					...
					: '')
			. '</div>';
		return apply_filters('diveit_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	add_shortcode("trx_tab", "diveit_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'diveit_sc_tabs_reg_shortcodes' ) ) {
	//add_action('diveit_action_shortcodes_list', 'diveit_sc_tabs_reg_shortcodes');
	function diveit_sc_tabs_reg_shortcodes() {
	
		diveit_sc_map("trx_tabs", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'diveit_sc_tabs_reg_shortcodes_vc' ) ) {
	//add_action('diveit_action_shortcodes_list_vc', 'diveit_sc_tabs_reg_shortcodes_vc');
	function diveit_sc_tabs_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Tabs extends Diveit_VC_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends Diveit_VC_ShortCodeTab {}
	}
}
?>