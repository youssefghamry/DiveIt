...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('diveit_sc_highlight_theme_setup')) {
	add_action( 'diveit_action_before_init_theme', 'diveit_sc_highlight_theme_setup' );
	function diveit_sc_highlight_theme_setup() {
		add_action('diveit_action_shortcodes_list', 		'diveit_sc_highlight_reg_shortcodes');
		if (function_exists('diveit_exists_visual_composer') && diveit_exists_visual_composer())
			add_action('diveit_action_shortcodes_list_vc','diveit_sc_highlight_reg_shortcodes_vc');
	}
}
...
if (!function_exists('diveit_sc_highlight')) {	
	function diveit_sc_highlight($atts, $content=null){	
		...
		$css .= ($color != '' ? 'color:' . esc_attr($color) . ';' : '')
			...
			.($font_size != '' ? 'font-size:' . esc_attr(diveit_prepare_css_value($font_size)) . '; line-height: 1em;' : '');
		$output = '<span' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				...
				. '</span>';
		return apply_filters('diveit_shortcode_output', $output, 'trx_highlight', $atts, $content);
	}
	add_shortcode('trx_highlight', 'diveit_sc_highlight');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'diveit_sc_highlight_reg_shortcodes' ) ) {
	//add_action('diveit_action_shortcodes_list', 'diveit_sc_highlight_reg_shortcodes');
	function diveit_sc_highlight_reg_shortcodes() {
	
		diveit_sc_map("trx_highlight", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'diveit_sc_highlight_reg_shortcodes_vc' ) ) {
	//add_action('diveit_action_shortcodes_list_vc', 'diveit_sc_highlight_reg_shortcodes_vc');
	function diveit_sc_highlight_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Highlight extends Diveit_VC_ShortCodeSingle {}
	}
}
?>