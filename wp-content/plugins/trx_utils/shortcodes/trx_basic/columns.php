...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('diveit_sc_columns_theme_setup')) {
	add_action( 'diveit_action_before_init_theme', 'diveit_sc_columns_theme_setup' );
	function diveit_sc_columns_theme_setup() {
		add_action('diveit_action_shortcodes_list', 		'diveit_sc_columns_reg_shortcodes');
		if (function_exists('diveit_exists_visual_composer') && diveit_exists_visual_composer())
			add_action('diveit_action_shortcodes_list_vc','diveit_sc_columns_reg_shortcodes_vc');
	}
}
...
if (!function_exists('diveit_sc_columns')) {	
	function diveit_sc_columns($atts, $content=null){	
		...
		diveit_storage_set('sc_columns_data', array(
			'counter' => 1,
			'equal_selector' => '',
            'after_span2' => false,
            'after_span3' => false,
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="columns_wrap sc_columns'
					. ' columns_' . (diveit_param_is_on($fluid) ? 'fluid' : 'nofluid') 
					. (!empty($margins) && diveit_param_is_off($margins) ? ' no_margins' : '') 
					. ' sc_columns_count_' . esc_attr($count)
					. (!empty($class) ? ' '.esc_attr($class) : '') 
				. '"'
				. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
				. (!diveit_param_is_off($equalheight) ? ' data-equal-height="'.esc_attr(diveit_storage_get_array('sc_columns_data', 'equal_selector')).'"' : '')
				. (!diveit_param_is_off($animation) ? ' data-animation="'.esc_attr(diveit_get_animation_classes($animation)).'"' : '')
				. '>'
					. trim($content)
				. '</div>';
		return apply_filters('diveit_shortcode_output', $output, 'trx_columns', $atts, $content);
	}
	add_shortcode('trx_columns', 'diveit_sc_columns');
}


if (!function_exists('diveit_sc_column_item')) {	
	function diveit_sc_column_item($atts, $content=null) {
		if (diveit_in_shortcode_blogger()) return '';
		extract(diveit_html_decode(shortcode_atts( array(
			// Individual params
			"span" => "1",
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' class="column-'.($span > 1 ? esc_attr($span) : 1).'_'.esc_attr(diveit_storage_get_array('sc_columns_data', 'count')).' sc_column_item sc_column_item_'.esc_attr(diveit_storage_get_array('sc_columns_data', 'counter')) 
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. (diveit_storage_get_array('sc_columns_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
					. (diveit_storage_get_array('sc_columns_data', 'counter') == 1 ? ' first' : '') 
					. ($span > 1 ? ' span_'.esc_attr($span) : '') 
					. (diveit_storage_get_array('sc_columns_data', 'after_span2') ? ' after_span_2' : '') 
					. (diveit_storage_get_array('sc_columns_data', 'after_span3') ? ' after_span_3' : '') 
					. (diveit_storage_get_array('sc_columns_data', 'after_span4') ? ' after_span_4' : '') 
					. '"'
					. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
					. (!diveit_param_is_off($animation) ? ' data-animation="'.esc_attr(diveit_get_animation_classes($animation)).'"' : '')
					. '>'
					...
					. '</div>';
		diveit_storage_inc_array('sc_columns_data', 'counter', $span);
		diveit_storage_set_array('sc_columns_data', 'after_span2', $span==2);
		diveit_storage_set_array('sc_columns_data', 'after_span3', $span==3);
		diveit_storage_set_array('sc_columns_data', 'after_span4', $span==4);
		diveit_storage_set_array('sc_columns_data', 'equal_selector', $bg_color!=='' || $bg_image !== '' ? '.sc_column_item_inner' : '.sc_column_item');
		return apply_filters('diveit_shortcode_output', $output, 'trx_column_item', $atts, $content);
	}
	add_shortcode('trx_column_item', 'diveit_sc_column_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'diveit_sc_columns_reg_shortcodes' ) ) {
	//add_action('diveit_action_shortcodes_list', 'diveit_sc_columns_reg_shortcodes');
	function diveit_sc_columns_reg_shortcodes() {
	
		diveit_sc_map("trx_columns", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'diveit_sc_columns_reg_shortcodes_vc' ) ) {
	//add_action('diveit_action_shortcodes_list_vc', 'diveit_sc_columns_reg_shortcodes_vc');
	function diveit_sc_columns_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Columns extends Diveit_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Column_Item extends Diveit_VC_ShortCodeCollection {}
	}
}
?>