...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('diveit_sc_slider_theme_setup')) {
	add_action( 'diveit_action_before_init_theme', 'diveit_sc_slider_theme_setup' );
	function diveit_sc_slider_theme_setup() {
		add_action('diveit_action_shortcodes_list', 		'diveit_sc_slider_reg_shortcodes');
		if (function_exists('diveit_exists_visual_composer') && diveit_exists_visual_composer())
			add_action('diveit_action_shortcodes_list_vc','diveit_sc_slider_reg_shortcodes_vc');
	}
}
...
if (!function_exists('diveit_sc_slider')) {	
	function diveit_sc_slider($atts, $content=null){	
		...
		extract(diveit_html_decode(shortcode_atts(array(
			...
			"slides_per_view" => 1,
			"titles" => "no",
			"descriptions" => diveit_get_custom_option('slider_info_descriptions'),
			"links" => "no",
			"align" => "",
			"interval" => "",
			"date_format" => "",
		...
		if (!empty($height) && diveit_param_is_on($autoheight)) $autoheight = "off";
		...
		$class2 = diveit_get_css_position_as_classes($top, $right, $bottom, $left);
		...
		$output = (in_array($pagination, array('full', 'over')) 
				...
				. ' class="sc_slider sc_slider_' . esc_attr($engine)
					...
					. (diveit_param_is_on($pagination) ? ' sc_slider_pagination' : ' sc_slider_nopagination')
					. (diveit_storage_get_array('sc_slider_data', 'bg_image') ? ' sc_slider_bg' : ' sc_slider_images')
					. (!in_array($pagination, array('full', 'over')) 
							? ($class2 ? ' '.esc_attr($class2) : '') . ($align!='' && $align!='none' ? ' align'.esc_attr($align) : '')
							: '')
					. '"'
				. (!in_array($pagination, array('full', 'over')) && !diveit_param_is_off($animation) ? ' data-animation="'.esc_attr(diveit_get_animation_classes($animation)).'"' : '')
		...
		if ($engine=='revo') {
			...
			$output .= '<div class="slides'
				...
				.($engine=='swiper' && diveit_storage_get_array('sc_slider_data', 'bg_image') ? ' style="'.esc_attr($hs).'"' : '')
				.'>';
			...
			if (diveit_param_is_on($custom) && $content) {
				...
				$args = diveit_query_add_filters($args, 'thumbs');
				...
				while ( $query->have_posts() ) { 
					...
					if (diveit_param_is_on($crop)) {
						$post_attachment = diveit_storage_get_array('sc_slider_data', 'bg_image')
							? diveit_get_resized_image_url($post_attachment, !empty($width) && (float) $width.' ' == $width.' ' ? $width : null, !empty($height) && (float) $height.' ' == $height.' ' ? $height : null)
							: diveit_get_resized_image_tag($post_attachment, !empty($width) && (float) $width.' ' == $width.' ' ? $width : null, !empty($height) && (float) $height.' ' == $height.' ' ? $height : null);
					} else if (!diveit_storage_get_array('sc_slider_data', 'bg_image')) {
					...
					if (in_array($pagination, array('full', 'over'))) {
						$old_output = $output;
						$output = '';
						if (file_exists(diveit_get_file_dir('templates/_parts/widgets-posts.php'))) {
					...
					$output .= '<div' 
						...
						. ' style="'
							. (diveit_storage_get_array('sc_slider_data', 'bg_image') ? 'background-image:url(' . esc_url($post_attachment) . ');' : '') . ($ws) . ($hs)
							. '"'
						. '>' 
						. (diveit_param_is_on($links) ? '<a href="'.esc_url($post_link).'" title="'.esc_attr($post_title).'">' : '')
						. (!diveit_storage_get_array('sc_slider_data', 'bg_image') ? $post_attachment : '')
						;
					$caption = $engine=='swiper' ? '' : $caption;
					if (!diveit_param_is_off($titles)) {
						$post_hover_bg  = diveit_get_scheme_color('text_link');
						$post_bg = '';
						if ($post_hover_bg!='' && !diveit_is_inherit_option($post_hover_bg)) {
							$rgb = diveit_hex2rgb($post_hover_bg);
						...
						$post_descr = diveit_get_post_excerpt();
						if (diveit_get_custom_option("slider_info_category")=='yes') { // || empty($cat)) {
							// Get all post's categories
							$post_tax = diveit_get_taxonomy_categories_by_post_type($post_type);
							if (!empty($post_tax)) {
								$post_terms = diveit_get_terms_by_post_id(array('post_id'=>$post_id, 'taxonomy'=>$post_tax));
								if (!empty($post_terms[$post_tax])) {
									...
									if ($post_category!='') {
										$caption .= '<div class="sc_slider_category"'.(diveit_substr($post_accent_color, 0, 1)=='#' ? ' style="background-color: '.esc_attr($post_accent_color).'"' : '').'><a href="'.esc_url($post_category_link).'">'.($post_category).'</a></div>';
									}
								}
							}
						}
						$output_reviews = '';
						if (diveit_get_custom_option('show_reviews')=='yes' && diveit_get_custom_option('slider_info_reviews')=='yes') {
							$avg_author = diveit_reviews_marks_to_display(get_post_meta($post_id, diveit_storage_get('options_prefix').'_reviews_avg'.((diveit_get_theme_option('reviews_first')=='author' && $orderby != 'users_rating') || $orderby == 'author_rating' ? '' : '2'), true));
							if ($avg_author > 0) {
								$output_reviews .= '<div class="sc_slider_reviews post_rating reviews_summary blog_reviews' . (diveit_get_custom_option("slider_info_category")=='yes' ? ' after_category' : '') . '">'
									. '<div class="criteria_summary criteria_row">' . trim(diveit_reviews_get_summary_stars($avg_author, false, false, 5)) . '</div>'
									. '</div>';
							}
						}
						if (diveit_get_custom_option("slider_info_category")=='yes') $caption .= $output_reviews;
						$caption .= '<h3 class="sc_slider_subtitle"><a href="'.esc_url($post_link).'">'.($post_title).'</a></h3>';
						if (diveit_get_custom_option("slider_info_category")!='yes') $caption .= $output_reviews;
						if ($descriptions > 0) {
							$caption .= '<div class="sc_slider_descr">'.trim(diveit_strshort($post_descr, $descriptions)).'</div>';
						}
						$caption .= '</div>';
					}
					$output .= ($engine=='swiper' ? $caption : '') . (diveit_param_is_on($links) ? '</a>' : '' ) . '</div>';
				}
				wp_reset_postdata();
			}
		...
		return apply_filters('diveit_shortcode_output', $output, 'trx_slider', $atts, $content);
	}
	add_shortcode('trx_slider', 'diveit_sc_slider');
}


if (!function_exists('diveit_sc_slider_item')) {	
	function diveit_sc_slider_item($atts, $content=null) {
		if (diveit_in_shortcode_blogger()) return '';
		extract(diveit_html_decode(shortcode_atts( array(
			// Individual params
			"src" => "",
		...
		$engine = diveit_storage_get_array('sc_slider_data', 'engine');
		$crop_image = diveit_storage_get_array('sc_slider_data', 'crop_image');
		$bg_image = diveit_storage_get_array('sc_slider_data', 'bg_image');
		$links = diveit_storage_get_array('sc_slider_data', 'links');
		$width = diveit_storage_get_array('sc_slider_data', 'width');
		$height = diveit_storage_get_array('sc_slider_data', 'height');
	
		if ($src && diveit_param_is_on($crop_image)) {
			$src = diveit_storage_get_array('sc_slider_data', 'bg_image')
				? diveit_get_resized_image_url($src, !empty($width) && diveit_strpos($width, '%')===false ? $width : null, !empty($height) && diveit_strpos($height, '%')===false ? $height : null)
				: diveit_get_resized_image_tag($src, !empty($width) && diveit_strpos($width, '%')===false ? $width : null, !empty($height) && diveit_strpos($height, '%')===false ? $height : null);
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '').' class="'.esc_attr($engine).'-slide' . (!empty($class) ? ' '.esc_attr($class) : '') . '"'
				. ($css ? ' style="'.esc_attr($css).'"' : '')
				.'>' 
				. ($src && diveit_param_is_on($links) ? '<a href="'.esc_url($src).'">' : '')
				. ($src && !$bg_image ? $src : $content)
				. ($src && diveit_param_is_on($links) ? '</a>' : '')
			. '</div>';
		return apply_filters('diveit_shortcode_output', $output, 'trx_slider_item', $atts, $content);
	}
	add_shortcode('trx_slider_item', 'diveit_sc_slider_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'diveit_sc_slider_reg_shortcodes' ) ) {
	//add_action('diveit_action_shortcodes_list', 'diveit_sc_slider_reg_shortcodes');
	function diveit_sc_slider_reg_shortcodes() {
	
		diveit_sc_map("trx_slider", array(
			...
			"params" => array_merge(array(
				"engine" => array(
					...
					"value" => "swiper",
					"type" => "checklist",
					"options" => diveit_get_sc_param('sliders')
				),
				"align" => array(
				...
				function_exists('diveit_exists_revslider') && diveit_exists_revslider() ? array(
				"alias" => array(
					...
					"value" => "",
					"type" => "select",
					"options" => diveit_get_sc_param('revo_sliders')
				)) : array(), array(
				"cat" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'diveit_sc_slider_reg_shortcodes_vc' ) ) {
	//add_action('diveit_action_shortcodes_list_vc', 'diveit_sc_slider_reg_shortcodes_vc');
	function diveit_sc_slider_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array_merge(array(
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(diveit_get_sc_param('sliders')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "align",
				...
				array(
					...
					"value" => array_flip(diveit_array_merge(array('none' => esc_html__('- Select slider -', 'trx_utils')), diveit_get_sc_param('revo_sliders'))),
					"type" => "dropdown"
				)) : array(), array(
				array(
					"param_name" => "cat",
		...
		class WPBakeryShortCode_Trx_Slider extends Diveit_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Slider_Item extends Diveit_VC_ShortCodeSingle {}
	}
}
?>