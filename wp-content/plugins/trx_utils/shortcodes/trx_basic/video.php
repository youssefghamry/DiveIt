...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('diveit_sc_video_theme_setup')) {
	add_action( 'diveit_action_before_init_theme', 'diveit_sc_video_theme_setup' );
	function diveit_sc_video_theme_setup() {
		add_action('diveit_action_shortcodes_list', 		'diveit_sc_video_reg_shortcodes');
		if (function_exists('diveit_exists_visual_composer') && diveit_exists_visual_composer())
			add_action('diveit_action_shortcodes_list_vc','diveit_sc_video_reg_shortcodes_vc');
	}
}
...
if (!function_exists('diveit_sc_video')) {	
	function diveit_sc_video($atts, $content = null) {
		...
		if (empty($height) && empty($width)) {
			$width='100%';
			if (diveit_param_is_off(diveit_get_custom_option('substitute_video'))) $height="400";
		}
		$ed = diveit_substr($width, -1);
		...
		$css_bg = diveit_get_css_paddings_from_values($bg_top, $bg_right, $bg_bottom, $bg_left);
	
		if ($src=='' && $url=='' && isset($atts[0])) {
			$src = $atts[0];
		}
		$url = $src!='' ? $src : $url;
		if ($image!='' && diveit_param_is_off($image))
			$image = '';
		else {
			if (diveit_param_is_on($autoplay) && is_singular() && !diveit_storage_get('blog_streampage'))
				$image = '';
			else {
				if ($image > 0) {
				...
				if ($bg_image) {
					...
					if (!is_single() || !empty($image)) $image = diveit_get_resized_image_url(empty($image) ? get_the_ID() : $image, $thumb_sizes['w'], $thumb_sizes['h'], null, false, false, false);
				} else
					if (!is_single() || !empty($image)) $image = diveit_get_resized_image_url(empty($image) ? get_the_ID() : $image, $ed!='%' ? $width : null, $height);
				if (empty($image) && (!is_singular() || diveit_storage_get('blog_streampage')))	// || diveit_param_is_off($autoplay)))
					$image = diveit_get_video_cover_image($url);
			}
		}
		if ($bg_image > 0) {
		...
		$url = diveit_get_video_player_url($src!='' ? $src : $url);
		...
		$video = '<video' . ($id ? ' id="' . esc_attr($id) . '"' : '') 
			...
			. (($image && diveit_param_is_on(diveit_get_custom_option('substitute_video'))) || (diveit_param_is_on($autoplay) && is_singular() && !diveit_storage_get('blog_streampage')) ? ' autoplay="autoplay"' : '') 
			. ' controls="controls" loop="loop"'
			. '>'
			. '</video>';
		if (diveit_param_is_off(diveit_get_custom_option('substitute_video'))) {
			if (diveit_param_is_on($frame)) $video = diveit_get_video_frame($video, $image, $css, $css_bg);
		} else {
		...
		return apply_filters('diveit_shortcode_output', $video, 'trx_video', $atts, $content);
	}
	add_shortcode("trx_video", "diveit_sc_video");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'diveit_sc_video_reg_shortcodes' ) ) {
	//add_action('diveit_action_shortcodes_list', 'diveit_sc_video_reg_shortcodes');
	function diveit_sc_video_reg_shortcodes() {
	
		diveit_sc_map("trx_video", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'diveit_sc_video_reg_shortcodes_vc' ) ) {
	//add_action('diveit_action_shortcodes_list_vc', 'diveit_sc_video_reg_shortcodes_vc');
	function diveit_sc_video_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Video extends Diveit_VC_ShortCodeSingle {}
	}
}
?>