...
define( 'TRX_SOCIALS_WIDGET_INSTAGRAM_CACHE_TIME', 12 * 60 * 60 );


// Get recent photos
if ( !function_exists( 'trx_socials_widget_instagram_get_recent_photos' ) ) {
	function trx_socials_widget_instagram_get_recent_photos($args) {
		// Check photos in the cache
		$client_id = ! empty( $args['demo'] ) ? 'demo' : trx_socials_widget_instagram_get_client_id();
		$cache_data = sprintf( 'trx_socials_instagram_data_%1$s_%2$s',
								$client_id,
		...
		if ( ! is_array($data) || ( ! empty($data['data']) && is_array($data['data']) && count($data['data']) < $args['count'] ) ) {
			...
			if ( ! empty( $args['demo'] ) ) {
				...
				$data = array(
					'data' => trx_socials_widget_instagram_demo_images( $args )
				);

			// Real mode - get images from Instagram
			} else {

				$access_token = get_option('trx_socials_api_instagram_access_token');
				
				// Get Instagram photos via Facebook API with sandbox application
				if ( ! empty( $access_token ) && empty( $args['hashtag'] ) ) {
					$access_token_valid = get_transient('trx_socials_instagram_access_token_valid');
					if ( ! $access_token_valid ) {
						$access_token = trx_socials_widget_instagram_refresh_access_token( $access_token );
					}
					$count = max(1, $args['count']);
					$resp = trx_socials_remote_get( 'https://graph.instagram.com/me/media'
														. '?fields=id,media_type,media_url,thumbnail_url,caption,username,timestamp'
														. "&access_token={$access_token}"
													);
					if (substr($resp, 0, 1) == '{') {
						$data = array(
							'data' => trx_socials_widget_instagram_parse_page_output( $resp, $args )
						);
					}
				...
				// Get Instagram photos via direct GET (parse html output)
					...
					$data = array(
						'data' => trx_socials_widget_instagram_get_page_output( $args )
					);
				}
			}
			...
			if ( is_array($data) && is_array($data['data']) && count($data['data']) > 0 ) {
				set_transient($cache_data, $data, TRX_SOCIALS_WIDGET_INSTAGRAM_CACHE_TIME);
			}
		}
...
// Register endpoints
if ( !function_exists( 'trx_socials_widget_instagram_rest_register_endpoints' ) ) {
	add_action( 'rest_api_init', 'trx_socials_widget_instagram_rest_register_endpoints');
	function trx_socials_widget_instagram_rest_register_endpoints() {
		// Get access token from Instagram
		register_rest_route( 'trx_socials/v1', '/widget_instagram/get_access', array(
			'methods' => 'GET,POST',
			'callback' => 'trx_socials_widget_instagram_rest_get_access',
...
if ( !function_exists( 'trx_socials_widget_instagram_rest_get_redirect_uri' ) ) {
	function trx_socials_widget_instagram_rest_get_redirect_uri() {
		$client_id  = get_option('trx_socials_api_instagram_client_id');
		$return_url = trx_socials_widget_instagram_rest_get_return_url();
...
if ( ! function_exists( 'trx_socials_widget_instagram_rest_get_return_url' ) ) {
	function trx_socials_widget_instagram_rest_get_return_url() {
		return trailingslashit( home_url() ) . "wp-json/trx_socials/v1/widget_instagram/get_access/";
	}
}


// Callback: Get authorization code from Instagram
if ( !function_exists( 'trx_socials_widget_instagram_rest_get_access' ) && class_exists( 'WP_REST_Request' ) ) {
	function trx_socials_widget_instagram_rest_get_access(WP_REST_Request $request) {
		...
		if ( empty($params['error']) && ! empty($params['state']) && ! empty( $nonce ) && $params['state'] == $nonce ) {
			...
			if ( empty( $access_token ) && ! empty( $code ) ) {
				$client_id = trx_socials_widget_instagram_get_client_id();
				$client_secret = trx_socials_widget_instagram_get_client_secret();
				// Request for short-time access token
				$resp = trx_socials_remote_post('https://api.instagram.com/oauth/access_token',
												...
												array(		// Request vars: post to server as request body
													...
													'code' => $code,
													'response_type' => 'code',
													'redirect_uri' => trx_socials_widget_instagram_rest_get_redirect_uri()
												)
										);
				if (substr($resp, 0, 1) == '{') {
					...
					if ( ! empty( $resp['access_token'] ) ) {
						...
						$resp = trx_socials_remote_get( 'https://graph.instagram.com/access_token'
			...
			if ( ! empty( $access_token ) ) {
				update_option('trx_socials_api_instagram_access_token', $access_token);
				if ( ! empty( $user_id ) ) {
					update_option('trx_socials_api_instagram_user_id', $user_id);
				}
				set_transient('trx_socials_instagram_access_token_valid', 1, $access_token_valid_time );
			}
		}		
		
		// Redirect to the options page
		wp_redirect(get_admin_url(null, 'themes.php?page=trx_socials_options'));
		die();
	}
}

// Refresh long-time access token
if( ! function_exists( 'trx_socials_widget_instagram_refresh_access_token' ) ) {
	function trx_socials_widget_instagram_refresh_access_token( $access_token ) {
		$resp = trx_socials_remote_get( 'https://graph.instagram.com/refresh_access_token'
		...
		if ( substr($resp, 0, 1) == '{' ) {
			...
			if ( ! empty( $resp['access_token'] ) ) {
				$access_token = $resp['access_token'];
				set_transient('trx_socials_instagram_access_token_valid', 1, max( 2 * 24 * 60 * 60, ! empty( $resp['expires-in'] ) ? $resp['expires-in'] - 2 * 24 * 60 * 60 : 0 ) );
...
// Register widget-specific options
if (!function_exists('trx_socials_widget_instagram_register_settings')) {
	add_action( 'admin_init', 'trx_socials_widget_instagram_register_settings' );
	function trx_socials_widget_instagram_register_settings() {
		global $TRX_SOCIALS_STORAGE;
		register_setting( 'trx_socials_settings_instagram', 'trx_socials_api_instagram_client_id' );
		register_setting( 'trx_socials_settings_instagram', 'trx_socials_api_instagram_client_secret' );
		register_setting( 'trx_socials_settings_instagram', 'trx_socials_api_instagram_access_token' );
		register_setting( 'trx_socials_settings_instagram', 'trx_socials_api_instagram_user_id' );
	}
}
...
if ( ! function_exists( 'trx_socials_widget_instagram_get_page_output' ) ) {
	function trx_socials_widget_instagram_get_page_output( $args ) {
		...
		$output = trx_socials_remote_get( $url, array(
			'headers' => array(
		...
		return empty( $output ) ? false : trx_socials_widget_instagram_parse_page_output( $output, $args );
	}
}

// Parse output from Instagram public feed
if( ! function_exists( 'trx_socials_widget_instagram_parse_page_output' ) ) {
	function trx_socials_widget_instagram_parse_page_output( $output, $args ) {
		...
		if ( substr($output, 0, 1) == '{' ) {
			...
			if ( ! empty( $output['data'] ) && is_array( $output['data'] ) ) {
				...
				$user_id = get_option('api_instagram_user_id');
					...
					$instagram[] = array(
						...
						'caption'	=> array(
											'text' => ! empty( $image['caption'] )
														? $image['caption']
														: esc_html__( 'Instagram Image', 'trx_socials' )
											),
						'user'		=> array(
...
if ( !function_exists( 'trx_socials_widget_instagram_ajax_load_images' ) ) {
	add_action('wp_ajax_trx_socials_instagram_load_images',			'trx_socials_widget_instagram_ajax_load_images');
	add_action('wp_ajax_nopriv_trx_socials_instagram_load_images',	'trx_socials_widget_instagram_ajax_load_images');
	function trx_socials_widget_instagram_ajax_load_images() {

		trx_socials_verify_nonce();

		$response = array( 'error' => '', 'data' => '' );
	
		$output = trx_socials_get_value_gp( 'output' );
		$hash   = str_replace( array( ' ', "\t", "\r", "\n" ), '', trx_socials_get_value_gp( 'hash' ) );

		if ( ! empty( $output ) ) {
			if ( $hash ) {
				$args = get_transient( sprintf( 'trx_socials_instagram_args_%s', $hash ) );
				if ( is_array($args) && ! empty( $args['hashtag'] ) ) {
					$data = array(
						'data' => trx_socials_widget_instagram_parse_page_output( $output, $args )
					);
					if ( is_array($data) && is_array($data['data']) && count($data['data']) > 0 ) {
						set_transient( sprintf( 'trx_socials_instagram_data_%1$s_%2$s',
													trx_socials_widget_instagram_get_client_id(),
													$args['hashtag']
												),
										$data,
										TRX_SOCIALS_WIDGET_INSTAGRAM_CACHE_TIME
										);
					}
					ob_start();
					...
					$response['error'] = esc_html__( 'Instagram Widget misconfigured, check widget settings.', 'trx_socials' );
				}
			} else {
				$response['error'] = esc_html__( 'Invalid hash value.', 'trx_socials' );
			}
		} else {
			$response['error'] = esc_html__( 'Get items from the Public Feed failed. Malformed data structure.', 'trx_socials' );
		}

		trx_socials_ajax_response( $response );
	}
}
...
//------------------------------------------------
if( !function_exists( 'trx_socials_widget_instagram_demo_images' ) ) {
	function trx_socials_widget_instagram_demo_images( $args ) {
		$instagram = array();
		$list = array();
		$titles = array();
		$url = '';
		...
		if ( ! empty( $args['demo_files'] ) && is_array( $args['demo_files'] ) && count( $args['demo_files'] ) > 0
			...
			foreach( $args['demo_files'] as $file ) {
				if ( ! empty( $file['image'] ) ) {
					$list[] = trx_socials_get_attachment_url(
									$file['image'],
					...
					$titles[] = trx_socials_get_attachment_caption(
									(int) $file['image'] > 0
										? $file['image']
										: ( ! empty( $file['image_extra']['id'] )
											? $file['image_extra']['id']
											: trx_socials_attachment_url_to_postid( $file['image'] )
											)
								);
			...
			$dir = trx_socials_get_folder_dir( TRX_SOCIALS_PLUGIN_WIDGETS . 'instagram/demo' );
			$url = trx_socials_get_folder_url( TRX_SOCIALS_PLUGIN_WIDGETS . 'instagram/demo' );
			$list = ! empty($dir) ? list_files($dir, 1) : array();
		}
		if ( is_array($list) ) {
			foreach ( $list as $k => $v) {
				$ext  = trx_socials_get_file_ext( $v );
				$type = in_array( $ext, array( 'mp4', 'mpg', 'avi' ) ) ? 'video' : 'image';
				$size = $type == 'image'
							? trx_socials_getimagesize( $v )
							: array( 1280, 720 );
				$thumb = array(
				...
				$instagram[] = array(
					...
					'user'		=> array(
										'username'	=> esc_html__( 'Demo Instagram User', 'trx_socials' ),
										'id' 		=> '',
										),
