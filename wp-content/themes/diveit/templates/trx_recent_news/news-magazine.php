...
if ( !function_exists( 'diveit_template_news_magazine_theme_setup' ) ) {
	add_action( 'diveit_action_before_init_theme', 'diveit_template_news_magazine_theme_setup', 1 );
	function diveit_template_news_magazine_theme_setup() {
		diveit_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'diveit'),
...
// Template output
if ( !function_exists( 'diveit_template_news_magazine_output' ) ) {
	function diveit_template_news_magazine_output($post_options, $post_data) {
		...
		?><article id="post-<?php echo esc_html($post_data['post_id']); ?>" 
				...
				<div class="post_header entry-header">
					...
					if ($show_title) {
						if (!isset($post_options['links']) || $post_options['links']) {
							?>
							<<?php echo esc_attr($title_tag); ?> class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php diveit_show_layout($post_data['post_title']); ?></a></<?php echo esc_attr($title_tag); ?>>
							<?php
						} else {
							?>
							<<?php echo esc_attr($title_tag); ?> class="post_title entry-title"><?php diveit_show_layout($post_data['post_title']); ?></<?php echo esc_attr($title_tag); ?>>
							<?php
						}
					}
					
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						?><div class="post_meta"><span class="post_meta_author"><?php diveit_show_layout($post_data['post_author_link']); ?></span><?php
			...
			if ($number <= $featured) {
					...
					if ($post_data['post_protected']) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(diveit_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
				...
				<div class="post_footer entry-footer">
					...
					<?php
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						$post_options['counters'] = 'views,comments,edit,captions';								diveit_template_set_args('counters', array(
							'post_options' => $post_options,
							'post_data' => $post_data
						));
