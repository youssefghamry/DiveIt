...
if ( !function_exists( 'diveit_template_gallery_theme_setup' ) ) {
    add_action( 'diveit_action_before_init_theme', 'diveit_template_gallery_theme_setup', 1 );
    function diveit_template_gallery_theme_setup() {
        diveit_add_template(array(
            'layout' => 'gallery_2',
            'template' => 'gallery',
            'mode'   => 'blog',
            'need_isotope' => true,
            'title'  => esc_html__('Gallery tile with preview mode /2 columns/', 'diveit'),
        ...
        diveit_add_template(array(
            ...
            'mode'   => 'blog',
            'need_isotope' => true,
            'title'  => esc_html__('Gallery tile /3 columns/', 'diveit'),
            'thumb_title'  => esc_html__('Medium image', 'diveit'),
            'w'		 => 370,
            'h_crop' => 209,
            'h'		 => null
        ...
        diveit_add_template(array(
            ...
            'mode'   => 'blog',
            'need_isotope' => true,
            'title'  => esc_html__('Gallery tile /4 columns/', 'diveit'),
            'thumb_title'  => esc_html__('Medium image', 'diveit'),
            'w'		 => 370,
            'h_crop' => 209,
            'h'		 => null
        ...
        add_action('diveit_action_blog_scripts', 'diveit_template_gallery_add_scripts');
    }
}

// Add template specific scripts
if (!function_exists('diveit_template_gallery_add_scripts')) {
        function diveit_template_gallery_add_scripts($style) {
        if (diveit_substr($style, 0, 8) == 'gallery_') {
            wp_enqueue_script( 'isotope', diveit_get_file_url('js/jquery.isotope.min.js'), array(), null, true );
            wp_enqueue_script( 'classie', diveit_get_file_url('js/core.gallery/classie.js'), array(), null, true );
            wp_enqueue_script( 'imagesloaded', diveit_get_file_url('js/core.gallery/imagesloaded.js'), array(), null, true );
            wp_enqueue_script( 'diveit-gallery-script', diveit_get_file_url('js/core.gallery/core.gallery.js'), array(), null, true );
            wp_enqueue_style(  'diveit-gallery-style', diveit_get_file_url('css/core.gallery.css'), array(), null );
        }
    }
}

// Template output
if ( !function_exists( 'diveit_template_gallery_output' ) ) {
    function diveit_template_gallery_output($post_options, $post_data) {
        ...
        $original_size = !empty($post_data['post_attachment']) ? diveit_getimagesize($post_data['post_attachment']) : array(0,0);
        ?>
        ...
        <div class="post_content isotope_item_content">
            <div class="post_featured <?php echo esc_attr($post_data['post_type'] == 'matches' ? 'matches_hover' : '');?>"><?php
                ...
                <div class="post_details">
                    ...
                    ?>
                    <h2 class="post_title"><?php diveit_show_layout($post_data['post_title'], $link_start, $link_end); ?></h2>
                    <div class="post_description">
                        ...
                        if ($post_data['post_link'] != '') {
                            ...
                            if ((!isset($post_options['readmore']) || !diveit_param_is_off($post_options['readmore'])) && !in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status'))) {
                                ?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_button sc_button_style_filled post_readmore"><span class="post_readmore_label"><?php echo !empty($post_options['readmore']) ? trim($post_options['readmore']) : esc_html__('Learn more', 'diveit'); ?></span></a><?php
                            }
                            ?></p><?php
                        }
                        ?>
