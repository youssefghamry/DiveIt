...
function diveit_theme_customizer(option, val) {
"use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = DIVEIT_STORAGE['co_add_params']!=undefined ? DIVEIT_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#diveit-customizer-styles-'+option).length > 0 ? jQuery('#diveit-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="diveit-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
