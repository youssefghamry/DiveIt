...
if ( !function_exists( 'diveit_core_customizer_theme_setup' ) ) {
	add_action( 'diveit_action_before_init_theme', 'diveit_core_customizer_theme_setup', 1 );
	function diveit_core_customizer_theme_setup() {

		// Load Color schemes then Theme Options are loaded
		add_action('diveit_action_load_main_options',					'diveit_core_customizer_load_options');

		// Recompile LESS and save CSS
		add_action('diveit_action_compile_less',						'diveit_core_customizer_compile_less');
		add_filter('diveit_filter_prepare_less',						'diveit_core_customizer_prepare_less');

		if ( is_admin() ) {
	
			// Ajax Save and Export Action handler
			add_action('wp_ajax_diveit_options_save', 				'diveit_core_customizer_save_options');

	
			// Ajax Delete color scheme Action handler
			add_action('wp_ajax_diveit_options_scheme_delete', 		'diveit_core_customizer_scheme_delete');


			// Ajax Copy color scheme Action handler
			add_action('wp_ajax_diveit_options_scheme_copy', 			'diveit_core_customizer_scheme_copy');
...
if ( !function_exists( 'diveit_core_customizer_theme_setup2' ) ) {
	add_action( 'diveit_action_before_init_theme', 'diveit_core_customizer_theme_setup2', 11 );
	function diveit_core_customizer_theme_setup2() {

		if ( is_admin() ) {

			// Add Theme Options in WP menu
			add_action('admin_menu', 								'diveit_core_customizer_admin_menu_item');
		}
...
if ( !function_exists( 'diveit_core_customizer_admin_menu_item' ) ) {
		function diveit_core_customizer_admin_menu_item() {
		diveit_admin_add_menu_item('theme', array(
			'page_title' => esc_html__('Fonts & Colors', 'diveit'),
			'menu_title' => esc_html__('Fonts & Colors', 'diveit'),
			'capability' => 'manage_options',
			'menu_slug'  => 'diveit_options_customizer',
			'callback'   => 'diveit_core_customizer_page',
			'icon'		 => ''
			)
		);
	}
...
if ( !function_exists( 'diveit_core_customizer_load_options' ) ) {
	function diveit_core_customizer_load_options() {
		$mode = isset($_POST['mode']) ? sanitize_text_field($_POST['mode']) : '';
		$override = isset($_POST['override']) ? sanitize_text_field($_POST['override']) : '';
		if ($mode!='reset' || $override!='customizer') {
			$storage = get_option( diveit_storage_get('options_prefix') . '_options_custom_colors' );
			if (!empty($storage)) diveit_storage_set('custom_colors', $storage);
			$storage = get_option( diveit_storage_get('options_prefix') . '_options_custom_fonts' );
			if (!empty($storage)) diveit_storage_set('custom_fonts', $storage);
		}
	}
}
...
// Ajax Save and Export Action handler
if ( !function_exists( 'diveit_core_customizer_save_options' ) ) {
			function diveit_core_customizer_save_options() {
		...
		if ( !wp_verify_nonce( diveit_get_value_gp('nonce'), admin_url('admin-ajax.php') ) || !current_user_can('manage_options'))
			wp_die();
                ...
                parse_str(diveit_get_value_gp('data'), $data);

		// Refresh array with schemes from POST data
		$colors = diveit_storage_get('custom_colors');
		if ($mode == 'save') {
			if (is_array($colors) && count($colors) > 0) {
				$order = !empty($data['diveit_options_schemes_order']) ? explode(',', $data['diveit_options_schemes_order']) : array_keys($colors);
				$schemes = array();
				foreach ($order as $slug) {
					$new_slug = $data[$slug.'-slug'];
					if (empty($new_slug)) $new_slug = diveit_get_slug($scheme['title']);
				...
				$colors = apply_filters('diveit_filter_save_custom_colors', $schemes);
				diveit_storage_set('custom_colors', $colors);
				update_option( diveit_storage_get('options_prefix') . '_options_custom_colors', $colors);
			}
		} else if ($mode == 'reset') {
			delete_option( diveit_storage_get('options_prefix') . '_options_custom_colors');
		}
		...
		if ($mode == 'save') {
					...
					if (is_array($font) && count($font) > 0) {
						foreach ($font as $key=>$value) {
							if (isset($data[$slug.'-'.$key]))
								$fonts[$slug][$key] = diveit_is_inherit_option($data[$slug.'-'.$key]) ? '' : $data[$slug.'-'.$key];
						}
					}
				}
			...
			delete_option( diveit_storage_get('options_prefix') . '_options_custom_fonts');
		}
...
// Ajax Delete color scheme Action handler
if ( !function_exists( 'diveit_core_customizer_scheme_delete' ) ) {
			function diveit_core_customizer_scheme_delete() {
		...
		if (isset($colors[$scheme])) {
			if (count($colors) > 1) {
				...
				$response['error'] = sprintf(esc_html__('You cannot delete last color scheme!', 'diveit'), $scheme);
		} else
			$response['error'] = sprintf(esc_html__('Color Scheme %s not found!', 'diveit'), $scheme);
...
// Ajax Copy color scheme Action handler
if ( !function_exists( 'diveit_core_customizer_scheme_copy' ) ) {
			function diveit_core_customizer_scheme_copy() {
			...
			foreach ($order as $slug) {
				if (is_array($colors[$slug]) && count($colors[$slug]) > 0) {
					...
					if ($slug == $scheme) {
						...
						$schemes[$new_slug]['title'] .= ' '.esc_html__('(Copy)', 'diveit');
					}
				}
			}
			$schemes = apply_filters('diveit_filter_save_custom_colors', $schemes);
...
if (!function_exists('diveit_core_customizer_compile_less')) {
		function diveit_core_customizer_compile_less() {
		if (diveit_get_theme_setting('less_compiler')=='no') return;
		$files = array();
		if (file_exists(diveit_get_file_dir('css/_utils.less'))) 	$files[] = diveit_get_file_dir('css/_utils.less');
		$files = apply_filters('diveit_filter_compile_less', $files);
		if (count($files) > 0) diveit_compile_less($files);
	}
}
...
if ( !function_exists( 'diveit_core_customizer_page' ) ) {
	function diveit_core_customizer_page() {
		...
		if (is_array($colors) && count($colors) > 0) {
			...
			$demo_block = '';
            if (diveit_get_theme_setting('customizer_demo') && file_exists(DIVEIT_FW_PATH . 'core/core.customizer/core.customizer.demo.php')) {
				ob_start();
                require_once DIVEIT_FW_PATH . 'core/core.customizer/core.customizer.demo.php';
				$demo_block = ob_get_contents();
				ob_end_clean();
			}
			$options["partition_schemes"] = array(
				"title" => esc_html__('Color schemes', 'diveit'),
				"override" => "customizer",
			...
			foreach ($colors as $slug=>$scheme) {
				...
				$options["{$slug}-description"] = array(
					"title" => sprintf(esc_html__('Color scheme "%s"', 'diveit'), $scheme['title']),
					"desc" => wp_kses_data( sprintf(__('Specify the color for each element in the scheme "%s". After that you will be able to use your color scheme for the entire page, any part thereof and/or for the shortcodes!', 'diveit'), $scheme['title']) ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-buttons_label"] = array(
					"desc" => wp_kses_data( __("You can duplicate current color scheme (appear on new tab) or delete it (if not last scheme)", 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "4_6 first",
					"type" => "label");
				...
				$options["{$slug}-button_delete"] = array(
					"title" => esc_html__('Delete',  'diveit'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "1_6 last",
				...
				if ($demo_block) {
					$options["{$slug}-demo"] = array(
						"title" => esc_html__('Usage demo', 'diveit'),
						"desc" => wp_kses_data( __('Below you can see the example of decoration of the page with selected colors.', 'diveit') )
									. trim($demo_block),
						"override" => "customizer",
						"type" => "info");
				}
...
if (isset($scheme['bg_color'])) {
				...
				$options["{$slug}-block_info"] = array(
					"title" => esc_html__('Page/Block decoration', 'diveit'),
					"desc" => wp_kses_data( __('Specify border and background to decorate whole page (if scheme accepted to the page) or entire block/section.', 'diveit') ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-bd_color_label"] = array(
					...
					"desc" => wp_kses_data( __('Select the border color and it hover state', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-bg_color_label"] = array(
					...
					"desc" => wp_kses_data( __('Select the background color and it hover state', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['bg_image'])) {
				...
				$options["{$slug}-bg_image_label"] = array(
					"title" => esc_html__('Background image', 'diveit'),
					"desc" => wp_kses_data( __('Select first background image and it display parameters', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-bg_image_position"] = array(
					"title" => esc_html__('Position', 'diveit'),
					"std" => "",
				...
				$options["{$slug}-bg_image_repeat"] = array(
					"title" => esc_html__('Repeat', 'diveit'),
					"std" => "",
					"val" => $scheme['bg_image_repeat'],
					"override" => "customizer",
					"divider" => false,
					"columns" => "1_5",
					"options" => diveit_get_list_bg_image_repeats(),
					"type" => "select");

				$options["{$slug}-bg_image_attachment"] = array(
					"title" => esc_html__('Attachment', 'diveit'),
					"std" => "",
					"val" => $scheme['bg_image_attachment'],
					"override" => "customizer",
					"divider" => false,
					"columns" => "1_5",
					"options" => diveit_get_list_bg_image_attachments(),
					"type" => "select");
	
				// Background image 2
				$options["{$slug}-bg_image2_label"] = array(
					"title" => esc_html__('Background image 2', 'diveit'),
					"desc" => wp_kses_data( __('Select second background image and it display parameters', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['accent2'])) {
				...
				$options["{$slug}-accent_info"] = array(
					"title" => esc_html__('Accented colors', 'diveit'),
					"desc" => wp_kses_data( __('Specify colors for the accented areas in your site.', 'diveit') ),
					"override" => "customizer",
					"type" => "info");

				// Accent 2 color
				$options["{$slug}-accent2_label"] = array(
					"title" => esc_html__('Accent 2', 'diveit'),
					"desc" => wp_kses_data( __('Select color for accented elements and their hover state', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['accent3'])) {
				...
				$options["{$slug}-accent3_label"] = array(
					"title" => esc_html__('Accent 3', 'diveit'),
					"desc" => wp_kses_data( __('Select color for accented elements and their hover state', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
...
if (isset($scheme['text'])) {
				...
				$options["{$slug}-text_info"] = array(
					"title" => esc_html__('Text and Headers', 'diveit'),
					"desc" => wp_kses_data( __('Specify colors for the plain text, post info blocks and headers', 'diveit') ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-text_label"] = array(
					...
					"desc" => wp_kses_data( __('Select colors for the text: normal text color, light text (for example - post info) and dark text (headers, bold text, etc.)', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['text_link'])) {
				...
				$options["{$slug}-text_link_label"] = array(
					"title" => esc_html__('Text links', 'diveit'),
					"desc" => wp_kses_data( __('Select color for the links and their hover state', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['inverse_text'])) {
				...
				$options["{$slug}-inverse_info"] = array(
					"title" => esc_html__('Inverse blocks', 'diveit'),
					"desc" => wp_kses_data( __('Specify colors for the headers, plain text, links and post info blocks in the accented areas (with background color equal to text link)', 'diveit') ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-inverse_label"] = array(
					...
					"desc" => wp_kses_data( __('Select colors for inversed text (text on accented background)', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-inverse_label2"] = array(
					"title" => esc_html__('Inverse links', 'diveit'),
					"desc" => wp_kses_data( __('Select colors for inversed links (links on accented background)', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['input_text'])) {
				...
				$options["{$slug}-input_info"] = array(
					"title" => esc_html__('Input colors: form fields and textareas', 'diveit'),
					"desc" => wp_kses_data( __('Specify colors to decorate input fields in the forms', 'diveit') ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-input_text_label"] = array(
					"title" => esc_html__('Text', 'diveit'),
					"desc" => wp_kses_data( __('Specify colors for the input fields for all states: disabled, inactive, active', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-input_text"] = array(
					"title" => esc_html__('Inactive', 'diveit'),
					"std" => "",
				...
				$options["{$slug}-input_light"] = array(
					"title" => esc_html__('Disabled', 'diveit'),
					"std" => "",
				...
				$options["{$slug}-input_dark"] = array(
					"title" => esc_html__('Active', 'diveit'),
					"std" => "",
				...
				$options["{$slug}-input_bd_color_label"] = array(
					"title" => esc_html__('Border color', 'diveit'),
					"desc" => wp_kses_data( __('Select the border colors for the normal state and for active (focused) field', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-input_bg_color_label"] = array(
					"title" => esc_html__('Background Color', 'diveit'),
					"desc" => wp_kses_data( __('Select the background colors for the normal state and for active (focused) field', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['alter_text'])) {
				...
				$options["{$slug}-alter_info"] = array(
					"title" => esc_html__('Alternative colors: Highlighted areas, submenu items, etc.', 'diveit'),
					"desc" => wp_kses_data( __('Specify colors to decorate highlighted blocks in the text, submenu items, etc.', 'diveit') ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-alter_text_label"] = array(
					"title" => esc_html__('Text and Headers', 'diveit'),
					"desc" => wp_kses_data( __('Specify colors for the plain text, post info blocks and headers in the highlight blocks', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_link_label"] = array(
					...
					"desc" => wp_kses_data( __('Specify colors for the links in the highlight blocks', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_bd_color_label"] = array(
					"title" => esc_html__('Border color', 'diveit'),
					"desc" => wp_kses_data( __('Select the border colors for the normal and hovered state', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_bg_color_label"] = array(
					"title" => esc_html__('Background Color', 'diveit'),
					"desc" => wp_kses_data( __('Select the background colors for the normal and hovered state', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['alter_bg_image'])) {
				...
				$options["{$slug}-alter_bg_image_label"] = array(
					"title" => esc_html__('Background image', 'diveit'),
					"desc" => wp_kses_data( __('Select alter background image and it display parameters', 'diveit') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
			...
			foreach ($fonts as $slug=>$font) {
				...
				if (isset($font['font-size'])) {
					$options["{$slug}-font-size"] = array(
						"title" => $show_titles ? esc_html__('Size', 'diveit') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['line-height'])) {
					$options["{$slug}-line-height"] = array(
						"title" => $show_titles ? esc_html__('Line height', 'diveit') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['font-weight'])) {
					$options["{$slug}-font-weight"] = array(
						"title" => $show_titles ? esc_html__('Weight', 'diveit') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['font-style'])) {
					$options["{$slug}-font-style"] = array(
						"title" => $show_titles ? esc_html__('Style', 'diveit') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['margin-top'])) {
					$options["{$slug}-margin-top"] = array(
						"title" => $show_titles ? esc_html__('Margin Top', 'diveit') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['margin-bottom'])) {
					$options["{$slug}-margin-bottom"] = array(
						"title" => $show_titles ? esc_html__('Margin Bottom', 'diveit') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
		...
		diveit_core_customizer_load_scripts();
		// Prepare javascripts global variables
		diveit_core_customizer_prepare_scripts();
		
		// Build Options page
		diveit_options_page_start(array(
			'title' => esc_html__('Fonts & Colors', 'diveit'),
			"icon" => "iconadmin-cog",
			"subtitle" => esc_html__('Fonts settings & Color schemes', 'diveit'),
			"description" => wp_kses_data( __('Customize fonts and colors for your site.', 'diveit') ),
			'data' => $options,
			'create_form' => true,
		...
		if (is_array($options) && count($options) > 0) {
			foreach ($options as $id=>$option) { 
				diveit_options_show_field($id, $option);
			}
		}
...
if (!function_exists('diveit_core_customizer_prepare_less')) {
		function diveit_core_customizer_prepare_less() {

		// Prefix for override rules
		$prefix = diveit_get_theme_setting('less_prefix');
		// Use nested selectors: increase .css size, but allow use nested color schemes
		$nested = diveit_get_theme_setting('less_nested');

		$out = '';

		// Custom fonts
		$fonts_list = diveit_get_list_fonts(false);
		...
		foreach ($custom_fonts as $slug => $font) {
			...
			if (!empty($font['font-family']) && !diveit_is_inherit_option($font['font-family']))
			...
			if (!empty($font['font-size']) && !diveit_is_inherit_option($font['font-size']))
				$out .= "@{$slug}_fs: " . diveit_prepare_css_value($font['font-size']) . ";\n";
			else
				$out .= "@{$slug}_fs: inherit;\n";
			
			if (!empty($font['line-height']) && !diveit_is_inherit_option($font['line-height']))
				$out .= "@{$slug}_lh: " . diveit_prepare_css_value($font['line-height']) . ";\n";
			else
				$out .= "@{$slug}_lh: inherit;\n";

			if (!empty($font['font-weight']) && !diveit_is_inherit_option($font['font-weight']))
				$out .= "@{$slug}_fw: " . trim($font['font-weight']) . ";\n";
			else
				$out .= "@{$slug}_fw: inherit;\n";

			if (!empty($font['font-style']) && !diveit_is_inherit_option($font['font-style']) && diveit_strpos($font['font-style'], 'i')!==false)
				$out .= "@{$slug}_fl: italic;\n";
			else
				$out .= "@{$slug}_fl: inherit;\n";

			if (!empty($font['font-style']) && !diveit_is_inherit_option($font['font-style']) && diveit_strpos($font['font-style'], 'u')!==false)
				$out .= "@{$slug}_td: underline;\n";
			else
				$out .= "@{$slug}_td: inherit;\n";

			if (!empty($font['margin-top']) && !diveit_is_inherit_option($font['margin-top']))
				$out .= "@{$slug}_mt: " . diveit_prepare_css_value($font['margin-top']) . ";\n";
			else
				$out .= "@{$slug}_mt: inherit;\n";

			if (!empty($font['margin-bottom']) && !diveit_is_inherit_option($font['margin-bottom']))
				$out .= "@{$slug}_mb: " . diveit_prepare_css_value($font['margin-bottom']) . ";\n";
			else
			...
			if (!empty($font['font-size']) && !diveit_is_inherit_option($font['font-size']))
				$out .= "\tfont-size:" . diveit_prepare_css_value($font['font-size']) . ";\n";
			if (!empty($font['line-height']) && !diveit_is_inherit_option($font['line-height']))
				$out .= "\tline-height: " . diveit_prepare_css_value($font['line-height']) . ";\n";
			if (!empty($font['font-weight']) && !diveit_is_inherit_option($font['font-weight']))
			...
			$out .= ".{$slug}_margins() {\n";
			if (!empty($font['margin-top']) && !diveit_is_inherit_option($font['margin-top']))
				$out .= "\tmargin-top: " . diveit_prepare_css_value($font['margin-top']) . ";\n";
			if (!empty($font['margin-bottom']) && !diveit_is_inherit_option($font['margin-bottom']))
				$out .= "\tmargin-bottom: " . diveit_prepare_css_value($font['margin-bottom']) . ";\n";
			$out .= "}\n\n";
		}
		}
		...
		$custom_colors = diveit_get_custom_colors();
		if (is_array($custom_colors) && count($custom_colors) > 0) {
			foreach ($custom_colors as $scheme => $data) {
				if (is_array($data) && count($data) > 0) {
					foreach ($data as $key => $value) {
						if ($key == 'title' || diveit_strpos($key, 'bg_image')!==false) continue;
						$out .= "@{$scheme}_{$key}: " . esc_attr(
							!empty($value) 
								? $value
								: (diveit_strpos($key, 'bg_image')!==false
									? 'none'
									: 'inherit'
									)
							) . ";\n";
...
if (!function_exists('diveit_core_customizer_load_scripts')) {
	function diveit_core_customizer_load_scripts() {
		if (file_exists(diveit_get_file_dir('core/core.customizer/core.customizer.css')))
			wp_enqueue_style( 'diveit-core-customizer-style',	diveit_get_file_url('core/core.customizer/core.customizer.css'), array(), null);
		if (file_exists(diveit_get_file_dir('core/core.customizer/core.customizer.js')))
            wp_enqueue_script( 'diveit-core-customizer-script', diveit_get_file_url('core/core.customizer/core.customizer.js'), array(), null, true );
	}
}


// Prepare javascripts global variables for customizer admin page
if ( !function_exists( 'diveit_core_customizer_prepare_scripts' ) ) {
	function diveit_core_customizer_prepare_scripts() {
		diveit_storage_set_array2('js_vars', 'to_strings', 'scheme_delete', 			esc_html__("Delete color scheme", 'diveit'));
		diveit_storage_set_array2('js_vars', 'to_strings', 'scheme_delete_confirm',	esc_html__("Do you really want to delete this color scheme?", 'diveit'));
		diveit_storage_set_array2('js_vars', 'to_strings', 'scheme_delete_complete',	esc_html__("Current color scheme is successfully deleted!", 'diveit'));
		diveit_storage_set_array2('js_vars', 'to_strings', 'scheme_delete_failed',	esc_html__("Error while delete color scheme! Try again later.", 'diveit'));
		diveit_storage_set_array2('js_vars', 'to_strings', 'scheme_copy', 			esc_html__("Copy color scheme", 'diveit'));
		diveit_storage_set_array2('js_vars', 'to_strings', 'scheme_copy_confirm', 	esc_html__("Duplicate this color scheme?", 'diveit'));
		diveit_storage_set_array2('js_vars', 'to_strings', 'scheme_copy_complete', 	esc_html__("Current color scheme is successfully duplicated!", 'diveit'));
		diveit_storage_set_array2('js_vars', 'to_strings', 'scheme_copy_failed', 		esc_html__("Error while duplicate color scheme! Try again later.", 'diveit'));
	}
}
...
if ( !function_exists( 'diveit_get_custom_fonts' ) ) {
	function diveit_get_custom_fonts() {
		return apply_filters('diveit_filter_get_custom_fonts', !diveit_storage_empty('custom_fonts') ? diveit_storage_get('custom_fonts') : array());
	}
}

// Add custom font parameters
if (!function_exists('diveit_add_custom_font')) {
	function diveit_add_custom_font($key, $data) {
		if (diveit_storage_empty('custom_fonts', $key)) diveit_storage_set_array('custom_fonts', $key, $data);
	}
}

// Return one or all font settings
if (!function_exists('diveit_get_custom_font_settings')) {
	function diveit_get_custom_font_settings($key, $param_name='') {
		return diveit_storage_get_array('custom_fonts', $key, $param_name);
	}
}

// Return fonts for css generator
if ( !function_exists( 'diveit_get_custom_fonts_properties' ) ) {
	function diveit_get_custom_fonts_properties() {
		$fnt = diveit_get_custom_fonts();
		$rez = array();
		foreach ($fnt as $k=>$f) {
			foreach ($f as $prop=>$val) {
				if ($prop == 'font-style') {
					if (diveit_strpos($val, 'i')!==false)
						$rez[$k.'_fl'] = 'italic';
					if (diveit_strpos($val, 'u')!==false)
						$rez[$k.'_td'] = 'underline';
				} else {
					$p = str_replace(
						array(
...
// Return fonts for css generator
if ( !function_exists( 'diveit_get_custom_font_css' ) ) {
	function diveit_get_custom_font_css($fnt) {
		$css = '';
		$fnt = diveit_storage_get_array('custom_fonts', $fnt);
		if (is_array($fnt)) {
			foreach ($fnt as $prop=>$val) {
				if (empty($val) || (diveit_strpos($prop, 'font-')===false && diveit_strpos($prop, 'line-')===false)) continue;
				if ($prop=='font-style') {
					if (diveit_strpos($val, 'i')!==false)
						$css .= ($css ? ';' : '') . $prop . ':italic';
					if (diveit_strpos($val, 'u')!==false)
...
// Return fonts for css generator
if ( !function_exists( 'diveit_get_custom_margins_css' ) ) {
	function diveit_get_custom_margins_css($fnt) {
		$css = '';
		$fnt = diveit_storage_get_array('custom_fonts', $fnt);
		if (is_array($fnt)) {
			foreach ($fnt as $prop=>$val) {
				if (empty($val) || diveit_strpos($prop, 'margin-')===false) continue;
				$css .= ($css ? ';' : '') . $prop . ':' . $val;
			}
		}
		return $css;
...
// Add color scheme
if (!function_exists('diveit_add_color_scheme')) {
	function diveit_add_color_scheme($key, $data) {
		if (diveit_storage_empty('custom_colors', $key)) diveit_storage_set_array('custom_colors', $key, $data);
	}
}

// Return color schemes
if ( !function_exists( 'diveit_get_custom_colors' ) ) {
	function diveit_get_custom_colors() {
		return apply_filters('diveit_filter_get_custom_colors', !diveit_storage_empty('custom_colors') ? diveit_storage_get('custom_colors') : array());
	}
}

// Return color schemes list, prepended inherit
if ( !function_exists( 'diveit_get_list_color_schemes' ) ) {
	function diveit_get_list_color_schemes($prepend_inherit=false) {
		$list = array();
		$colors = diveit_storage_get('custom_colors');
...
// Return scheme color
if (!function_exists('diveit_get_scheme_color')) {
	function diveit_get_scheme_color($clr_name='', $clr='') {
		if (empty($clr)) {
			$scheme = diveit_get_custom_option('body_scheme');
			if (empty($scheme) || diveit_storage_empty('custom_colors', $scheme)) $scheme = 'original';
			$clr = diveit_storage_get_array('custom_colors', $scheme, $clr_name);
		}
		return apply_filters('diveit_filter_get_scheme_color', $clr, $clr_name, $scheme);
	}
}

// Return scheme colors
if (!function_exists('diveit_get_scheme_colors')) {
	function diveit_get_scheme_colors($scheme='') {
		if (empty($scheme)) $scheme = diveit_get_custom_option('body_scheme');
		if (empty($scheme) || diveit_storage_empty('custom_colors', $scheme)) $scheme = 'original';
		return diveit_storage_get_array('custom_colors', $scheme);
	}
}
?>