...
( function( $ ) {
	...
	if (!diveit_previewer_vars['need_refresh']) {
		var $style = $('#diveit-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="diveit-customizer-css"></'+'style'+'>' ).find('#diveit-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			diveit_set_cookie(diveit_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
